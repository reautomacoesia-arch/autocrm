...
export default function OnboardingTab({ clientId }: OnboardingTabProps) {
  ...
  return (
    <form onSubmit={handleSave} className="max-w-2xl space-y-4">
      ...
      <div className="mb-4 p-3 bg-[#1a1a1d] border border-slate-700 rounded-lg">
      ...
      {FIELDS.map(({ key, label, placeholder, rows }) => (
        <div key={key}>
          ...
          <textarea
            ...
            rows={rows}
            className="w-full bg-[#1a1a1d] border border-slate-700 text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-indigo-500 resize-none"
          />
        </div>
      ))}
      <div className="pt-2">
